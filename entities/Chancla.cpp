...
void actualizarChancla(Juego* juego) {
    ...
    if (!c.regresando) {
        ...
        float speed = CHANCLA_VELOCIDAD * 1.2f;
    ...
    // CORRECCIÓN BUG: después de golpear un enemigo, salir del loop
    // para evitar quedarse colisionando infinitamente con el enemigo
    ...
    for (int i = 0; i < juego->enemigosActivos; i++) {
        ...
        if (SDL_HasRectIntersectionFloat(&hitbox, &en->rect)) {
            ...
            if (en->vida <= 0) {
                ...
                float ey = en->rect.y;
                // Usar callback con frase de chancla
                mundoOnEnemigoMuertoChancla(juego, i, ex, ey);
            }
            // Salir del loop tras el primer golpe del frame —
            // evita que colisione con el enemigo recién regenerado
            break;
        }
    }
    ...
    if (juego->estadoBoss == BOSS_ACTIVO || juego->estadoBoss == BOSS_ENFURECIDO) {
        for (int p = 0; p < MAX_PILARES; p++) {
            ...
            if (SDL_HasRectIntersectionFloat(&hitbox, &juego->pilares[p].rect)) {
                ...
                break;  // mismo patrón: un golpe por frame
            }
        }
    }
...
void renderizarChancla(Juego* juego) {
    ...
    for (int i = PASOS; i >= 1; i--) {
        ...
        SDL_FPoint ctr  = {SZ * 0.5f, SZ * 0.5f};
        SDL_RenderTextureRotated(r, tex, NULL, &dst, rot, &ctr, SDL_FLIP_NONE);
    }

    // Sprite principal
    SDL_SetTextureColorMod(tex, 255, 255, 255);
    SDL_SetTextureAlphaMod(tex, 255);
    SDL_FRect dstMain  = {c.x - SZ * 0.5f, c.y - SZ * 0.5f, SZ, SZ};
    SDL_FPoint ctrMain = {SZ * 0.5f, SZ * 0.5f};
    SDL_RenderTextureRotated(r, tex, NULL, &dstMain,
        (double)c.angulo, &ctrMain, SDL_FLIP_NONE);
    ...
    SDL_FRect glow = {c.x - SZ * 0.8f, c.y - SZ * 0.8f, SZ * 1.6f, SZ * 1.6f};
...
void renderizarBarraCooldownChancla(Juego* juego) {
    ...
    Uint8 rr = 255, gg = (Uint8)(progreso * 200), bb = 0;
